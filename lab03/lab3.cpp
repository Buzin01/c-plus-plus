...
void Triangle::print()
{
    ...
    std::cout << "Perimeter: " << perimeter() << " Square: " << square() << std::endl;
}
...
double TetrahedralPrism::perimeter() {return distance3D(points3D[0], points3D[1]) * 8 + distance3D(points3D[3], points3D[4]) * 4;}
double TetrahedralPrism::volume() {return pow(distance3D(points3D[0], points3D[1]), 2) * distance3D(points3D[3], points3D[4]);}
